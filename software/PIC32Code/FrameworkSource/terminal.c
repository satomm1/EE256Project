...
{    
    // Pin A0 is the UART TX --- Digital Output
    TRISACLR = _TRISA_TRISA0_MASK;
    ANSELACLR = _ANSELA_ANSA0_MASK;
    LATAbits.LATA0 = 1; // Start with TX high
    
    // Pin C1 is the UART RX -- Digital Input
    TRISCSET = _TRISC_TRISC1_MASK;
    ANSELCCLR = _ANSELC_ANSC1_MASK;
    
    RPA0R = 0b00001; // map U1TX -> RA0
    U1RXR = 0b0110; // map U1RX <- RC1
